<template>
  <section class="flex flex-col p-8 items-center overflow-x-hidden overflow-y-scroll">
    <div class="flex flex-row items-center justify-start mb-4 space-x-4 mb-4 mt-8">
      <img class="w-12" src="/Smiling face with sunglasses.svg">
      <h1 class="text-black font-extrabold tracking-tight text-4xl w-full flex flex-row items-start lg:items-center">
        Upgrade and go pro!
      </h1>
    </div>
    ...
    >
      <p class="text-black opacity-70 font-semibold">
        Successfully {{ billing.cardNumber ? "saved" : "cleared" }} billing information!
      </p>
    </div>
    ...
    <div class="flex flex-col p-6 bg-white shadow rounded-2xl justify-center items-start w-full mb-8">
      <ul
        class="flex flex-col items-center justify-center text-center p-6 w-full bg-opaqueIndigo border-2 border-gdp font-bold mb-8 rounded-2xl text-gdp"
      >
        <h3 class="font-extrabold text-xl text-blackish mb-2">
    ...
    async saveBillingInfo() {
      ...
      try {
        await this.$axios.$post('/payments/set-card-info', {
          ...
          creditCard: {
            number: this.billing.cardNumber,
            expiration: this.billing.expDate,
            cvc: this.billing.securityCode
          }
        });
